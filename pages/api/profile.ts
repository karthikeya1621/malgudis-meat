...
async function handler(req: any, res: any) {
    ...
    if (action === 'update_info') {
        ...
        axios.put(`${process.env.BIGCOMMERCE_STORE_API_URL}/v3/customers`, [{ ...customerData }], {
            headers: {
        ...
        axios.get(`${process.env.BIGCOMMERCE_STORE_API_URL}/v3/customers/addresses?${(new URLSearchParams(queries)).toString()}`, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'X-Auth-Token': `${process.env.BIGCOMMERCE_STORE_API_TOKEN}`
